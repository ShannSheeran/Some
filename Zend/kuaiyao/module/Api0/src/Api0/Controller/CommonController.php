...
use Api0\Controller\Common\Login;
use Api0\Controller\Common\Structure;
use Api0\Controller\Common\Request;
use Api0\Controller\Common\TableRequest;
use Api0\Controller\Common\WhereRequest;
use Api0\Controller\Common\Response;
...
use Api0\Controller\Item\PushFromItem;
use Api0\Controller\Item\PushArgsItem;
use Api0\Controller\Item\PushTemplateItem;
...
//         $json = str_replace('null', '""', $json);
     ...
     * 获取Post过来的Query对象，防止
     *
     * @return \Api0\Controller\Common\Request
     */
     ...
     *
     * @return \Api0\Controller\Common\TableRequest
     */
     ...
     *
     * @return \Api0\Controller\Common\WhereRequest
     */
     ...
     * 初始化输出对象
     *
     * @return \Api0\Controller\Common\Response
     */
...
//                 @file_put_contents($filename, $data);
     ...
     *
     * @param unknown $filename
     * @return Ambigous <boolean, \Api0\Controller\mixed, mixed>
     */
     ...
     * @return Ambigous <\Api0\Controller\multitype:multitype:multitype:multitype:unknown, multitype:multitype:multitype:multitype:unknown    multitype:string unknown  >
...
//         }
     ...
     * @return multitype:
     */
    public function getDeviceForUser($ids)
    {
        if (! is_array($ids))
        {
            $ids = array(
        ...
        foreach ($ids as $key => $id)
        {
            ...
            $temp = $this->getDeviceUserTable()->fetchAll($where,'user_id asc');
...
//                     $result = $push->pushCollectionDevice(array($value), $value->content, $value->title, $args);
     ...
     */
    public function pushForController($ids, $contentType, PushArgsItem $args = null, PushFromItem $from = null, PushTemplateItem $template = null)
    {
        ...
        $deviceCollection = $this->getDeviceForUser($ids); // 根据id和用户类型查找设备号与设备类型
        if (! $template)
        {
        ...
        if ($ids && $template->content)
        {
            ...
            foreach ($ids as $key => $id)
            {
                $data = array(
                    ...
                    'type' => isset($from) ? (int)$from->type : 0,
                    'status' => 2,
                    'parameter' => json_encode($template->push_args),
                    'foreign_id' => isset($from) ? $from->id : 0,
                    'user_id_from' => isset($from) ? (int)$from->user_id : 0,
                    'user_id' => $id,
        ...
        if ($deviceCollection && $template->content)
        {
            ...
            if (PUSH_LOG_SWITCH)
            {
                ...
                {
//                     $string_args = implode(",", (array)$args);
                    $content = "推送,".(2 == $value['notification'] ? '用户关闭推送,':'')." user_id :$value[user_id] , title : ".$template->title." , content : ".$template->content." , args：" . json_encode($template->push_args) . ", 设备号：$value[device_token]";
...
//                 $string_args = implode(",", (array)$args);
                if (! $deviceCollection)
                {
                    $content = "推送，msg：找不到对应的设备号 , 或对应设备号关闭推送功能 , user_id：" . $string_id  . " , args：" . json_encode($template->push_args) . ", 模版：" . $contentType;
                    $myfile->putAtStart($content);
                }
                if (! $template->content)
                {
                    $content = "推送 , msg：不能生成content , 检查模版类型和参数是否相对应 ,  user_id：" . $string_id . " , args：" . json_encode($template->push_args) . ", 模版：" . $contentType;
                    $myfile->putAtStart($content);
                }
            }
        }
