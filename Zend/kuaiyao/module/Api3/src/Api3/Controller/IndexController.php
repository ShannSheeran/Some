...
'18620042165');
    ...
    public function getAlipayNotifyAction()
    {
        ...
        $verify_result = $alipay->getAlipayNotifySuccess(2);    //  app
        if ($verify_result) { // 验证成功
            ...
            if ($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
                $this->getIndexController()->orderUpdateStatus($out_trade_no, 1);
...
//         $file->putAtStart("result:" . var_export($result,true));
        
        if (! $result['status'] && $result['out_trade_no'])
        {
            $out_trade_no = $result['out_trade_no'];
            $file->putAtStart("success:" . $out_trade_no);
