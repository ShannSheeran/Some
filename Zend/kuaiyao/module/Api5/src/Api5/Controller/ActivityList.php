...
use Api5\Controller\Request\ActivityListWhereRequest;
...
class ActivityList extends CommonController
{
    ...
    public function index()
    {
        ...
        if ($table_where->companyId) {
            $where->equalTo('com_id', $table_where->companyId);
        }
        ...
        if ($data) {
            foreach ($data['list'] as $val) {
                $imagePath = '';
                if ($val['logo']) {
                    $imagePath = $this->getImageTable()->getOne(array(
                        'id' => $val['logo']
                    ));
                    if ($imagePath) {
                ...
                $page_id = $this->getPageTable()->getOne(array(
