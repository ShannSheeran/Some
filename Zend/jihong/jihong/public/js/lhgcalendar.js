...
var calendar = function(b)
{
	...
	p.applnk( J.path() + 'lhgcalendar.css' ); J(p._main).acls('cal_panel');
...
J.exend( calendar.prototype, {
	...
	createbody: function(d,m)
	{
		...
		txt = '<div id="pm"></div>' + txt + '<div id="nm"></div>';
		
		J(chead).stcs({ width: '450px', height: '39px' ,background: '#c4234c',color:'#fff'}).html(txt); J(m).apch(chead);
		...
		var cbody = J(d).crte('div'); J(cbody).stcs({width:'448px',border:'1px solid #e6e6e6'}).html(txt); J(m).apch(cbody);
		
		J('pm',d).click(this.pm); J('nm',d).click(this.nm);
		J('year',d).click(this.year); J('month',d).click(this.month);
