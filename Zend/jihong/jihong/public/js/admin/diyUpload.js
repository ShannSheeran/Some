...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('error', function( code ) {
				...
				//$(".parentFileBox").prepend("<p>"+text+"</p>");
				$("<p>"+text+"</p>").appendTo(".parentFileBox");
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//添加子容器;
		if($(".webuploader-pick").html()=="选择公司图片"){
			var i_name="company_album[]";
		}
		else{
			var i_name="project_album[]";
		}
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					<div class="viewThumb"></div> \
					<div class="diyCancel">x</div> \
					<div class="diySuccess"></div> \
					<input type="hidden" id="file_'+file_id+'" name="'+i_name+'"> \
