...
class Image
{
	 ...
	 */
	function makeThumb($orgin, $thumb_width = 0, $thumb_height = 0, $path = NULL, $bgcolor = '#FFFFFF', $quality = 85,$type=2)
	{
		...
		if($thumb_width == 0)
		{
			$canvas_width = min($org_info[0], round($thumb_height * $src_scale)); // 画布宽
		}
		elseif($thumb_height == 0)
		{
			$canvas_height = min($org_info[1], round($thumb_width / $src_scale)); // 画布高
		}
	 ...
	 */
	function watermark($orgin, $target = '', $mark_type = 'txt', $water_img = '', $water_txt = '', $color = '#FF0000', $font = 14, $font_type = NULL, $pos = 9, $alpha = 65, $quality = 85)
	{
		if(is_array($orgin))
		{
			extract($orgin);
