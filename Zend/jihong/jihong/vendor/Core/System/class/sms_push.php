<?php
require_once PUSH_ROOT.'/sms/Client.php';

class sms_push {
	private $config_file = 'config/sms_config.php';
	private $url = '';
	...
	private function sendMessage($mobile_array,$content){
		...
		if($statusCode != 0){
			$this->myfile->put(sprintf("statusCode:".$statusCode." ".$client->getError(),$this->config_file),1);
			return false;
		}else {
			return true;
		}
